...
SELECT CLI.Nombre1, CLI.Apellido1, MEM.IdMembresia
FROM CLIENTE CLI
INNER JOIN MEMBRESIA MEM
ON MEM.Dni_Cliente = CLI.DniCliente
INNER JOIN MEMBRESIA_REGULAR 
ON MEMBRESIA_REGULAR.IdMembresia = MEM.IDMembresia;
...
--¿Cuántas clases tiene asiganado cada empleado?
WITH CANTIDAD_CLASES AS (SELECT INSTRUCTOR.DNI, COUNT(*) AS "Cantidad de clases"
FROM CLASE
INNER JOIN EMPLEADO
ON EMPLEADO.DNI = CLASE.DniEmpleado 
INNER JOIN INSTRUCTOR 
ON INSTRUCTOR.DNI = EMPLEADO.DNI
GROUP BY INSTRUCTOR.DNI)
SELECT CANTIDAD_CLASES.DNI, EMPLEADO.Nombre1, EMPLEADO.Nombre2, EMPLEADO.Apellido1, "Cantidad de clases"
FROM CANTIDAD_CLASES
INNER JOIN EMPLEADO
ON CANTIDAD_CLASES.DNI = EMPLEADO.DNI
ORDER BY "Cantidad de clases" DESC;

--¿Cuántas clases tiene asiganado el empleado con DNI = 08017242?
SELECT COUNT(*) AS "Cantidad de clases"
FROM CLASE
INNER JOIN EMPLEADO
ON EMPLEADO.DNI = CLASE.DniEmpleado 
INNER JOIN INSTRUCTOR 
ON INSTRUCTOR.DNI = EMPLEADO.DNI
WHERE INSTRUCTOR.DNI = '08017242';


--¿Qué clases imparte cada instructor y qué especialización tiene?
SELECT CLASE.IdClase, EMPLEADO.Nombre1, EMPLEADO.Nombre2, INSTRUCTOR.Especializacion_Ins
FROM CLASE
INNER JOIN EMPLEADO
ON EMPLEADO.DNI = CLASE.DniEmpleado 
INNER JOIN INSTRUCTOR 
...
--¿Cuál es el número de clases que tiene cada cliente?
WITH CLASES_CLIENTES AS(SELECT CLIENTE_RECIBE_CLASES.DniCliente,  COUNT(*)AS "Cantidad de clases"
FROM CLIENTE_RECIBE_CLASES
INNER JOIN CLIENTE
ON CLIENTE.DniCliente = CLIENTE_RECIBE_CLASES.DniCliente
GROUP BY CLIENTE_RECIBE_CLASES.DniCliente)
SELECT CLASES_CLIENTES.DniCliente, CLIENTE.Nombre1, CLIENTE.Apellido1,"Cantidad de clases"
FROM CLASES_CLIENTES
INNER JOIN CLIENTE
ON CLIENTE.DniCliente = CLASES_CLIENTES.DniCliente
ORDER BY "Cantidad de clases" DESC;


--¿Cuántos pagos se han realizado por cada membresía?
SELECT MENSUALIDAD.IdMembresia,COUNT(*) AS "Cantidad de pagos"
FROM MENSUALIDAD
GROUP BY MENSUALIDAD.IdMembresia
ORDER BY "Cantidad de pagos" DESC;


--¿Cuál es el valor total cancelado por cada membresía?
WITH CANTIDAD_PAGOS_POR_MEMBRESIA AS (SELECT MENSUALIDAD.IdMembresia,COUNT(*) AS "Cantidad de pagos"
FROM MENSUALIDAD
GROUP BY MENSUALIDAD.IdMembresia)
SELECT CANTIDAD_PAGOS_POR_MEMBRESIA.IdMembresia, MEMBRESIA.PRECIO*"Cantidad de pagos" AS "Valor total cancelado a la fecha"
FROM CANTIDAD_PAGOS_POR_MEMBRESIA
INNER JOIN MEMBRESIA
ON MEMBRESIA.IdMembresia = CANTIDAD_PAGOS_POR_MEMBRESIA.IdMembresia
ORDER BY "Valor total cancelado a la fecha" DESC;


--¿Cuántos clientes llevan entre 2 y 3 clases?
WITH CLASES_CLIENTES AS(SELECT CLIENTE_RECIBE_CLASES.DniCliente,  COUNT(*)AS "Cantidad de clases"
FROM CLIENTE_RECIBE_CLASES
INNER JOIN CLIENTE
ON CLIENTE.DniCliente = CLIENTE_RECIBE_CLASES.DniCliente
...
INNER JOIN CLIENTE
ON CLIENTE.DniCliente = CLASES_CLIENTES.DniCliente
WHERE "Cantidad de clases" BETWEEN 2 AND 3;


--¿Cuál es el cliente con membresía más antiguo?
SELECT CLIENTE.NOMBRE1, CLIENTE.Apellido1, MEMBRESIA.Fecha_Inscripcion
FROM CLIENTE
INNER JOIN MEMBRESIA
ON CLIENTE.DniCliente = MEMBRESIA.Dni_Cliente
ORDER BY MEMBRESIA.Fecha_Inscripcion ASC
FETCH FIRST 1 ROWS ONLY;




--¿Cuáles son los clientes que adquirieron membresías entre 2020 y 2021?
SELECT CLIENTE.NOMBRE1, CLIENTE.Apellido1, MEMBRESIA.Fecha_Inscripcion
FROM CLIENTE
INNER JOIN MEMBRESIA
ON CLIENTE.DniCliente = MEMBRESIA.Dni_Cliente
WHERE MEMBRESIA.Fecha_Inscripcion BETWEEN '01/01/20' AND '31/12/21'
ORDER BY MEMBRESIA.Fecha_Inscripcion ASC;

--¿Cuántos son los clientes que adquirieron membresías entre 2020 y 2021?
SELECT COUNT(*) AS "Cantidad de clientes del 2020 y 2021"
FROM CLIENTE
INNER JOIN MEMBRESIA
ON CLIENTE.DniCliente = MEMBRESIA.Dni_Cliente
WHERE MEMBRESIA.Fecha_Inscripcion BETWEEN '01/01/20' AND '31/12/21';
...
--¿Cuál es el total de venta en una fecha dada?
SELECT SUM(FACTURA.Total) AS "Total de mis ventas"
FROM FACTURA
WHERE FACTURA.Fecha = '01/04/21';




--¿Cuál es el cliente con membresía más antigua?
SELECT CLIENTE.NOMBRE1, CLIENTE.Apellido1, MEMBRESIA.Fecha_Inscripcion
FROM CLIENTE
INNER JOIN MEMBRESIA
ON CLIENTE.DniCliente = MEMBRESIA.Dni_Cliente
